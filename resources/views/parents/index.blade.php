    ...
    <div class="min-h-screen flex">
        ...
        <div class="flex-1 p-10 overflow-y-auto bg-gradient-to-br from-slate-50 to-indigo-50/50">
            ...
            <div class="flex justify-between items-center mb-10">
                ...
                <a href="{{ route('parents.create') }}" class="bg-gradient-to-r from-indigo-500 to-blue-600 hover:from-indigo-600 hover:to-blue-700 text-white font-bold py-3.5 px-6 rounded-xl shadow-lg transition-all duration-300 flex items-center transform hover:-translate-y-1 w-fit">
                    <i class="fa-solid fa-user-plus mr-2"></i> Add New Parent
                </a>
            </div>

            <form action="{{ route('parents.index') }}" method="GET" class="mb-8 bg-white p-3 pr-4 rounded-2xl shadow-md border border-slate-100 flex gap-3 items-center">
                <div class="relative flex-1 group">
                    ...
                    <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Search parents by name or contact number..." 
                ...
                @if($search)
                <a href="{{ route('parents.index') }}" class="bg-rose-50 hover:bg-rose-100 text-rose-600 px-6 py-3.5 rounded-xl text-sm font-bold transition-colors shadow-sm flex items-center">
                ...
                <table class="min-w-full text-left border-collapse">
                    ...
                    <tbody class="text-slate-700 text-sm divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-indigo-50/40 transition-colors duration-200">
                            <td class="px-8 py-5 font-bold text-slate-400">{{ $parent->parent_id }}</td>
                            <td class="px-8 py-5">
                                <div class="flex items-center">
                                    ...
                                    <span class="font-extrabold text-slate-800 text-base">{{ $parent->name }}</span>
                                </div>
                            </td>
                            <td class="px-8 py-5">
                                <div class="font-semibold text-slate-600 mb-1 flex items-center">
                                    <i class="fa-solid fa-phone text-xs text-slate-400 mr-2"></i> {{ $parent->contact }}
                                </div>
                                <div class="text-xs text-slate-500 font-medium flex items-center">
                                    <i class="fa-solid fa-envelope text-xs text-slate-400 mr-2"></i> 
                                    {{ $parent->email ? $parent->email : 'No email added' }}
                                </div>
                            </td>
                            <td class="px-8 py-5 text-slate-500 font-medium">{{ $parent->address ?? 'N/A' }}</td>

                            <td class="px-8 py-5">
                                <div class="flex items-center justify-center gap-3">
                                    <a href="{{ route('parents.show', $parent->parent_id) }}" class="text-white bg-gradient-to-r from-indigo-500 to-blue-600 hover:from-indigo-600 hover:to-blue-700 px-4 py-2.5 rounded-xl transition-all font-bold text-xs flex items-center shadow-md transform hover:-translate-y-0.5" title="View Dashboard">
                                        <i class="fa-solid fa-chart-pie mr-2"></i> Analytics
                                    </a>

                                    <a href="{{ route('parents.edit', $parent->parent_id) }}" class="text-amber-600 hover:text-white bg-amber-50 hover:bg-amber-500 px-3 py-2.5 rounded-xl transition-all font-bold text-sm flex items-center" title="Edit Parent">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>

                                    <form action="{{ route('parents.destroy', $parent->parent_id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this parent?');">
                                        @csrf
                                        @method('DELETE')
