        ...
        <div class="flex-1 p-10 overflow-y-auto bg-slate-50">
            ...
            <div class="flex justify-between items-center mb-8">
                ...
                <div class="flex gap-3">
                    <a href="{{ route('parents.index') }}" class="bg-white border border-slate-200 text-slate-700 hover:bg-slate-100 font-bold py-2.5 px-5 rounded-xl shadow-sm transition-all flex items-center text-sm">
                        <i class="fa-solid fa-arrow-left mr-2"></i> Back
                    </a>
                    <a href="{{ route('parents.report', $parent->parent_id) }}" class="bg-gradient-to-r from-indigo-500 to-blue-600 hover:from-indigo-600 hover:to-blue-700 text-white font-bold py-2.5 px-5 rounded-xl shadow-lg transition-all flex items-center text-sm transform hover:-translate-y-1">
                        <i class="fa-solid fa-file-pdf mr-2"></i> Generate Report
                    </a>

                    <form action="{{ route('parents.alert-email', $parent->parent_id) }}" method="POST" class="inline">
                        @csrf
            ...
            @if($student)
                <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 mb-8 flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
                    ...
                    <div class="flex items-center gap-6 pl-2">
                        ...
                        <div>
                            <h3 class="text-xl font-black text-slate-800 mb-3">Student: {{ $student->name }}</h3>
                            ...
                            <div class="flex flex-wrap items-center gap-3">
                                <div class="text-sm font-bold text-slate-600 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-lg flex items-center shadow-sm">
                                    <i class="fa-solid fa-user-shield text-indigo-500 mr-2"></i> {{ $parent->name }}
                                </div>
                                
                                <div class="text-sm font-bold text-slate-600 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-lg flex items-center shadow-sm">
                                    <i class="fa-solid fa-phone text-indigo-500 mr-2"></i> {{ $parent->contact }}
                                </div>
                    ...
                    <div class="bg-white p-8 rounded-3xl shadow-lg border border-slate-100 relative overflow-hidden flex flex-col">
                        ...
                        <div class="relative h-[220px] w-full flex-1 flex justify-center items-center">
                            ...
                            <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none mt-4">
                                <span class="text-4xl font-black text-slate-800">{{ number_format($student->attendance_percentage ?? 0, 0) }}%</span>
                        ...
                        <div class="mt-6 pt-6 border-t border-slate-100 flex justify-between">
                            <div class="text-center">
                                ...
                                <p class="text-lg font-black text-slate-700">{{ $student->attendances->count() }}</p>
                            </div>
                            <div class="text-center">
                                <p class="text-xs font-bold text-emerald-400">Attended</p>
                                <p class="text-lg font-black text-emerald-600">{{ $student->attendances->whereIn('status', ['Present', 'Late','present','late'])->count() }}</p>
                            </div>
                        </div>
                    </div>
