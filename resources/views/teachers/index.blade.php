    ...
    <div class="min-h-screen flex">
        ...
        <div class="flex-1 p-10 overflow-y-auto bg-slate-50">
            ...
            <div class="flex justify-between items-center mb-10">
                ...
                <a href="{{ route('teachers.create') }}" class="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold py-3.5 px-6 rounded-xl shadow-lg transition-all duration-300 flex items-center">
                    <i class="fa-solid fa-user-plus mr-2"></i> Add New Teacher
                </a>
            </div>

            <form action="{{ route('teachers.index') }}" method="GET" class="mb-8 bg-white p-3 pr-4 rounded-2xl shadow-md border border-slate-100 flex gap-3 items-center">
                ...
                @if(isset($search) && $search != '')
                    <a href="{{ route('teachers.index') }}" class="bg-red-50 text-red-600 px-6 py-3 rounded-xl font-bold text-sm">
                ...
                <table class="min-w-full text-left border-collapse">
                    ...
                    <tbody class="text-slate-700 text-sm divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-blue-50/40 transition">
                            <td class="px-8 py-5 font-bold text-slate-500">{{ $teacher->teacher_id }}</td>
                            <td class="px-8 py-5">
                                <div class="flex items-center">
                                    <div class="h-10 w-10 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center font-extrabold mr-4">{{ strtoupper(substr($teacher->name, 0, 1)) }}</div>
                                    <span class="font-extrabold text-slate-800">{{ $teacher->name }}</span>
                                </div>
                            </td>
                            <td class="px-8 py-5">{{ $teacher->contact }}</td>
                            <td class="px-8 py-5">
                                <span class="bg-indigo-50 text-indigo-700 text-xs font-extrabold px-4 py-2 rounded-xl border border-indigo-100">{{ $teacher->subject }}</span>
                            </td>
                            <td class="px-8 py-5">
                                <div class="flex justify-center gap-3">
                                    <a href="{{ route('teachers.edit', $teacher->teacher_id) }}" class="text-blue-600 hover:bg-blue-600 hover:text-white bg-blue-50 p-2.5 rounded-xl transition-all">
                                        <i class="fa-solid fa-pen-to-square w-4"></i>
                                    </a>
                                    <form action="{{ route('teachers.destroy', $teacher->teacher_id) }}" method="POST" onsubmit="return confirm('Delete this teacher?');">
                                        @csrf
                                        @method('DELETE')
