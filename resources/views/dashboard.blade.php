...
<body class="bg-slate-50 font-sans antialiased">
    ...
    @endif
            ...
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 mb-8 flex justify-between items-center">
                <div>
                    <!-- login user name -->
                    <h2 class="text-2xl font-black text-slate-800 tracking-tight">Welcome back, <span class="text-blue-600">{{ Auth::user()->name }}!</span></h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-all">
                    ...
                    <h3 class="text-4xl font-black text-slate-800">{{ $totalTeachers }}</h3>
                </div>
                ...
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-all">
                    ...
                    <h3 class="text-4xl font-black text-slate-800">{{ $totalStudents }}</h3>
                </div>
                ...
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-all">
                    ...
                    <h3 class="text-4xl font-black text-slate-800">{{ $totalClasses }}</h3>
                </div>
            </div>
                    ...
                    <div class="space-y-4 flex-1 overflow-y-auto">
                        ...
                        <div class="bg-rose-50 border-l-4 border-rose-500 p-4 rounded-r-xl">
                            ...
                            <p class="text-xs font-medium text-rose-600">
                                {{ $student->name }}'s attendance requires attention (Currently at <span class="font-black">{{ $student->attendance_percentage }}%</span>).
                            </p>

                            <a href="{{ route('dashboard.sendAlert', $student->student_id) }}" class="text-xs font-bold text-rose-700 mt-2 inline-block hover:underline">
                    ...
                    <div class="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                        ...
                        <div class="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-slate-50 p-4 rounded-2xl border border-slate-100 shadow-sm">
                            <div class="flex items-center justify-between mb-1">
                                ...
                                <span class="text-xs font-bold text-slate-400">{{ $teacher->created_at->diffForHumans() }}</span>
                            </div>
                            <p class="text-xs font-medium text-slate-500">{{ $teacher->name }} was registered into the system (Subject: {{ $teacher->subject }}).</p>
                        </div>
                    </div>
                    @empty
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const dynamicChartData = @json($chartData);
